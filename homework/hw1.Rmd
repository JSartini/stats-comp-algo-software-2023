...
**Your answer:** The size of the folder is not reasonable, as the `git_experiment` folder has size 8.9MB, as compared to the cumulative file size of less than 50KB. This is likely due to the .git folder storing compressed versions of both the source foo.tex and the much larger output foo.pdf at all 100 of the different commit versions, resulting in the observed directory inflation and providing an illustrative example for why non-source files should be avoided in git repositories where possible.  
...
[Here](https://github.com/JSartini/hiperglm) is the desired hyperlink.