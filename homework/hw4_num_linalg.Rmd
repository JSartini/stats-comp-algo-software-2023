...
**Response:**
Here, the LU decomposition is fastest with median time 233ms, followed by the LU decomposition with median time 453ms, and then finally QR decomposition, the slowest with median time 834ms. This ordering makes some sense given the complexity analyses undergone for these algorithms during lecture, where it was indicated that Cholesky decomposition has complexity of order $\frac{1}{3}p^3$, LU decomposition has complexity of order $\frac{2}{3}p^3$, and QR decomposition has order $\frac{4}{3}np^2 = \frac{4}{3}p^3$ for this $n=p$ case with $p$ parameters and $n$ number of data points. Note that, while the QR-decomposition complexity analysis performed in class was for solving least squares, this is actually algebraically identical to solving the system, as solving $Ax = b$ for $x$ and $A = QR$ implies calculating $R^{-1}Q^Ty$, the same formulation used to find coefficients in least squares. This means that the Cholesky decomposition should take about half of the time of the LU decomposition, which itself would take around half the time of the QR decomposition. However, none of these expected relationships appear as expected in my own testing. According to the results below, the Cholesky and LU decomposition-based methods actually swapped order, with LU being around 24% faster. Then, the QR decomposition, while indeed slower than the other two, does not take double the time of the LU-based method nor quadruple the time of the Cholesky-based method. These differences are likely due to optimizations under the hood for each of these methods, such that the full decompositions are likely not being calculated.
...
Note that the above plot is a multi-dimensional scaling of the $x$ vectors produced using each solving procedure. This plot projects the $x$ points in 1024-dimensional space down to 2-dimensions while preserving the distances between them. Do note that the y-axis is much finer than the x-axis here, so the best reference for the actual distances are the labels attached to each solution. The plot itself serves mostly for spatial orientation. According to this plot, the LU decomposition is the closest to the "ground truth" x vector, followed by the Cholesky decomposition, then finally the QR decomposition.
